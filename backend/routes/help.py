...
def create_help_request():
    ...
    return jsonify({"help_id": str(res.inserted_id), "status": "open"})
...
def teacher_list_requests():
    ...
    for r in help_requests_col.find({"status": status}).sort("created_at", -1):
        student = users_col.find_one({"_id": ObjectId(r["user_id"])}) if ObjectId.is_valid(r["user_id"]) else None
        lesson = lessons_col.find_one({"_id": ObjectId(r["lesson_id"])}) if ObjectId.is_valid(r["lesson_id"]) else None
        reqs.append({
            "help_id": str(r["_id"]),
            "status": r["status"],
            "message": r["message"],
            "reply": r.get("reply"),
            "student_name": student["name"] if student else r["user_id"],
            "lesson_title": lesson["title"] if lesson else r["lesson_id"],
